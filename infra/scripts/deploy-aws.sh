...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --profile)
      ...
      ;;
    -h|--help)
      echo "Usage: $0 [--local] [--region REGION] [--profile PROFILE] [instance-id]"
      echo ""
      echo "Modes:"
...
if $LOCAL_MODE; then
  ...
  DEPLOY_COMMANDS=$(cat <<CMDS
    ...
    "sudo -u ec2-user npm install --production --ignore-scripts",
    "sudo -u ec2-user npm run build",
    "if command -v docker &>/dev/null && docker info &>/dev/null; then cd container && ./build.sh && cd ..; else echo Skipping container build; fi",
